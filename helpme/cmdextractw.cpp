...
LPKCCMDLINEARGV extract_cmdline(wchar_t* cmdstr, int len)
{
	...
	*pn = _T('\0');
	...
	_ret = (LPKCCMDLINEARGV)malloc(sizeof(KCCMDLINEARGV)+sizeof(wchar_t*) * argc);
	_ret->argc = argc;
	_ret->argv[argc] = NULL;
	pn = cmdstr;
...
ULONG64 extract_digtial(wchar_t* str DBGONLY_PARAM(IDebugClient* Client))
{
	CComQIPtr<IDebugControl4> dbgCtrl = Client;
	DBGONLY_STATEMENT(dbgCtrl->OutputWide(DEBUG_OUTPUT_NORMAL, L"str is: %s", str);)
...
ULONG64 extract_address(wchar_t* str, IDebugClient* Client)
{
	...
	while(*ptr != _T('\0') && *ptr != _T('|') && *ptr != _T('!'))
		ptr++;
	DBGONLY_STATEMENT(dbgCtrl->OutputWide(DEBUG_OUTPUT_NORMAL, L"The input address is: %s, got symbol pointer: %s\n", str, ptr);)

	if(*ptr == _T('0')) {
		DBGONLY_STATEMENT(dbgCtrl->OutputWide(DEBUG_OUTPUT_NORMAL, L"Neither '!' nor '|' are found, treat as digital address format\n");)
		offset = extract_digtial(str DBGONLY_ARG(Client));
	}
	else {
		DBGONLY_STATEMENT(dbgCtrl->OutputWide(DEBUG_OUTPUT_NORMAL, L"Either '!' or '|' is found, treat as symbol address format\n");)
		if(_istdigit(*ptr)) {
			DBGONLY_STATEMENT(dbgCtrl->OutputWide(DEBUG_OUTPUT_NORMAL, L"The first character for symbol ptr is %c, tereat as digital address format.\n", *(ptr+1));)
			*ptr = _T('\0');
			dbgSym->GetModuleByModuleNameWide(str, 0, NULL, &offset);
			offset += extract_digtial(ptr+1 DBGONLY_ARG(Client));
			*ptr = _T('!');
		}
		else {
			DBGONLY_STATEMENT(dbgCtrl->OutputWide(DEBUG_OUTPUT_NORMAL, L"Treat as symbol address format.\n");)
			ULONG64 shan;
			*ptr = _T('!');
			dbgSym->StartSymbolMatchWide(str, &shan);
			dbgSym->GetNextSymbolMatchWide(shan, NULL, 0, NULL, &offset);
			dbgSym->EndSymbolMatch(shan);
		}
	}
	DBGONLY_STATEMENT(dbgCtrl->OutputWide(DEBUG_OUTPUT_NORMAL, L"The extracted address is %y\n", offset);)
	return offset;
}
